            ...
            for (var j = 0; j < y; j++) {
                ...
                for (var i = 0; i < x; i++) {
                    if (! highlighted || obj.records[j][i].classList.contains('highlight')) {
                        ...
                        if (! obj.options.columns[i].title) {
                            obj.options.columns[i].title = jexcel.helpers.getColumnName(i);
                        }
                        row[obj.options.columns[i].title] = obj.options.data[j][i];
                    }
                }
                    ...
                    if (obj.options.allowRenameColumn == true) {
                        items.push({
                            ...
                            onclick:function() {
                                ...
                                inputElement.value = obj.headers[x].title;
                                inputElement.setAttribute("style","width: 100%;");
                                obj.headers[x].innerHTML="";
                                obj.headers[x].appendChild(inputElement);
                                obj.resetSelection();
                                inputElement.focus();
                                inputElement.addEventListener("blur", (event) => {
                                    const nemName=event.currentTarget.value;
                                    event.currentTarget.parentElement.innerHTML="";
                                    obj.setHeader(x,nemName);
                                });
                            }
                        });
                    }
